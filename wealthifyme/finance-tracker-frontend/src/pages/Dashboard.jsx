...
const StatCard = ({ title, value, sub, subColor, icon, grad }) => (
  <div className="bg-white dark:bg-dark-card rounded-2xl p-5 border border-slate-200 dark:border-dark-border shadow-sm">
    <div className="flex items-start justify-between mb-3">
      <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{title}</p>
      <span className={`text-2xl p-1.5 rounded-lg bg-gradient-to-br ${grad} bg-opacity-20`}>{icon}</span>
    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-dark-card rounded-2xl p-5 border border-slate-200 dark:border-dark-border shadow-sm">
          <div className="flex items-start justify-between mb-3">
            ...
            <span className="text-2xl p-1.5 rounded-lg">🎯</span>
          </div>
          <p className="text-2xl font-bold text-slate-900 dark:text-white">{savingsRate}%</p>
          <p
            className={`text-xs mt-1 font-medium ${
              savingsRate >= 20 ? "text-teal-500" : savingsRate >= 0 ? "text-orange-500" : "text-red-500"
            }`}
          >
            {savingsRate >= 20 ? "▲ Great savings habit!" : savingsRate >= 0 ? "▲ On track" : "▼ Overspending"}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center text-base flex-shrink-0 ${
                      t.type === "income" ? "bg-teal-500/15 dark:bg-teal-500/10" : "bg-red-500/15 dark:bg-red-500/10"
                    }`}
                  >
                    {t.type === "income" ? "📥" : "📤"}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {t.householdId && (
                        <span className="text-[9px] bg-teal-500/10 text-teal-600 dark:text-teal-400 px-1 py-0.2 rounded font-bold">
                          🏠 Shared
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-bold ${t.type === "income" ? "text-teal-500" : "text-red-400"}`}>
                      {t.type === "income" ? "+" : "-"}₹{t.amount.toLocaleString("en-IN")}
                    </span>
                    <button
                      onClick={() => deleteTxn(t._id)}
                      className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-500 transition-all text-xs"
                    >
                      ✕
                    </button>
                  </div>
        ...
        <div className="space-y-6">
          ...
          {household ? (
            <div className="bg-white dark:bg-dark-card rounded-2xl border border-slate-200 dark:border-dark-border p-5 bg-gradient-to-br from-teal-500/[0.02] to-emerald-500/[0.02] shadow-sm">
            ...
            <div className="bg-white dark:bg-dark-card rounded-2xl border border-slate-200 dark:border-dark-border p-5 border-dashed flex flex-col items-center text-center">
              ...
              <button
                ...
                className="mt-4 w-full py-2.5 text-center rounded-xl bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-dark-border text-xs text-teal-600 dark:text-teal-400 hover:bg-slate-100 dark:hover:bg-white/10 font-bold transition-all"
              >
                Setup Household
              </button>
            </div>
